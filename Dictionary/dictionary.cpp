...
namespace sdds {
	...
	int LoadDictionary(const char* filename) {
		...
		if (openFileForRead(filename)) {
			...
			success = 0;
			freadDictionary(&record, filename); // Write dictionary to record if file found
			closeFile();

		}
		else {
			record = {}; // Set dictionary to empty if no file found
		}
	...
	void SaveDictionary(const char* filename) { // Open, write and close copy of current dictionary

		openFileForOverwrite(filename);

		fwriteDictionary(&record);
